...
| Sin commitear | **El texto literal de `t3` entra entero en este mismo commit:** `content/schema.ts` (el tope de `quote`, 320 → 400), `content/home.ts`, `docs/TESTIMONIOS_FUENTES.md`, `docs/CONTENT_TODO.md` y este fichero. Es el **segundo commit del mismo bloque**: el primero (`92be347`) publicó la reseña resumida a 280 y el guardarraíl de `tests/unit/content.test.ts`; el cliente prefirió sus palabras exactas, así que este segundo la deja literal. Queda suelto **sólo** `scripts/verify-staging-order.ts`, sin rastrear y ajeno a este bloque como a los cinco anteriores: nadie lo reclama, así que o se adopta o se borra. Comprobar con `git status --short`. Lo de abajo es el histórico de bloques ya commiteados. **La activación de Web Analytics** entró **entera en su propio commit** (`264ed2e`) (el hash no se duplica aquí, ver `git log -1 --oneline`) — `package.json` y `package-lock.json` (la dependencia `@vercel/analytics`), `lib/analytics.ts` (nuevo), `app/layout.tsx`, `content/pages.ts` (sólo la sección `cookies` del aviso legal), `tests/unit/analytics.test.ts` (nuevo), `tests/unit/pages.test.ts` y este fichero. Queda suelto **sólo** `scripts/verify-staging-order.ts`, sin rastrear; comprobar con `git status --short`. El bloque anterior, **el abono del 50%**, se commiteó entero en `3b76c0d` — `content/pages.ts`, `docs/boquita-sobre-nosotros.md`, `docs/CONTENT_TODO.md` y este fichero. Y antes, **el repaso de copy del catálogo**, también entero en `d8fbbd4` — `content/pages.ts`, `docs/boquita-sobre-nosotros.md` y este fichero. Y antes, el copy de la portada, también entero — `content/home.ts`, `lib/seo.ts`, `tests/unit/content.test.ts`, `tests/e2e/geometry.spec.ts` y este fichero. ⚠ Sigue suelto de antes `scripts/verify-staging-order.ts`, **sin rastrear y ajeno a todos los bloques**; nadie lo ha reclamado en cuatro bloques seguidos, así que o se adopta o se borra. Verificar con `git status --short`. Lo de abajo es el histórico de bloques ya commiteados, que se conserva porque documenta qué tocó cada uno. **El catálogo salado** (17 ago, commit `5eb146f`): `content/products.ts`, `content/home.ts`, `content/pages.ts`, `types/shop.ts`, `lib/homeContent.ts`, `scripts/build-images.mjs`, `app/tienda/page.tsx`, `app/layout.tsx`, `drizzle/0004_ordinary_thor_girl.sql` + su `_journal.json`, las tres fuentes de `assets/products/` y los nueve `.webp` de `public/img/producto/`, cinco ficheros de `tests/unit/`, dos de `tests/e2e/`, y `README.md`, `docs/IMAGE_MAP.md`, `docs/CONTENT_TODO.md` y este fichero. ⚠ En el mismo árbol sigue suelto de antes `scripts/verify-staging-order.ts`, sin rastrear, que **no es de este bloque**. Verificar con `git status --short`. Lo de abajo es la lista de bloques anteriores, ya commiteados: **El arreglo del nav**: `components/layout/Navbar.tsx`, `styles/10-navbar.css`, `tests/e2e/interactions.spec.ts`, `CLAUDE.md` (punto 10) y este fichero. ⚠ En el mismo árbol hay **otro bloque que no es de éste y no debe entrar en el mismo commit**: la tarjeta Open Graph y el ajuste de SEO — `app/opengraph-image.tsx`, `app/og-hero.jpg` (nuevo, sin rastrear), `scripts/build-images.mjs`, `next.config.ts`, `lib/seo.ts` (sale `streetAddress` del JSON-LD) y la `description` de `app/layout.tsx`—. Ese bloque **seguía creciendo mientras se escribía esto**, así que la lista se comprueba, no se copia. ⚠ Siguen sueltos de antes `scripts/verify-staging-order.ts` sin rastrear y `data/boquita_products_catalog.xlsx` modificado. Verificar con `git status --short` |
| Bloque reseñas reales + estrellas | `t1`, `t2` y `t3` de `content/home.ts` son reseñas **reales** de Ale sin marca `todo`: María Elena M. (Escazú) y Mirella S. (Santa Ana) el 15 ago, y **Vanessa Manco** (Piedades de Santa Ana) el **19 ago**, que sustituye a la inventada «Laura Céspedes Mora». `t4`…`t6` siguen siendo andamio. **`t3` se publica LITERAL**, y es lo que obligó a subir el tope de `quote` de 320 a **400** en `content/schema.ts`: son los dos últimos párrafos tal como los escribió, con su ❤️ —la única tarjeta con emoji—. Estuvo unas horas resumida a 280 y el cliente prefirió sus palabras exactas. Con 400 pasa a ser, con diferencia, la más larga. El `role` dice «Clienta + de 4 años, Piedades» y no el distrito completo por composición: con «Piedades de Santa Ana» pasaba de los 40 del esquema y salía en dos líneas a 992. Las seis tarjetas muestran **cinco estrellas de 32px en su propia línea**, entre el nombre y la ocasión (D-41), que **se rellenan de izquierda a derecha** al asomar la sección —de 15% a opaco, una cada 180ms, 1,24s en total, las seis tarjetas a la vez—. El disparo cuelga del `.slider` y reutiliza el observer compartido de `lib/revealObserver.ts`. El guardarraíl de `content.test.ts` deja de contar marcas y **nombra** las que faltan, hoy tres: `["t4","t5","t6"]`. Originales sin editar y estado del permiso en `docs/TESTIMONIOS_FUENTES.md`. No toca esquema, tipos, slider (más allá del disparo), base de datos ni SEO. |
...
| Tests unitarios | **348 en verde**, 20 ficheros (`npm test`, medido el 17 ago al cerrar la activación de Web Analytics), con `npm run lint` y `npm run typecheck` limpios. Los **+3 sobre 345** son los dos casos de `tests/unit/analytics.test.ts` —el fichero 20, la puerta de `VERCEL_ENV`— y el guardarraíl nuevo de `pages.test.ts`, que afirma que la sección `cookies` del aviso legal cuenta la medición y ya no presume de «ni analítica de terceros». Antes: **345 en verde**, 19 ficheros, medido al cerrar el copy de la portada y **vuelto a medir** al cerrar el repaso de copy del catálogo, que no añade ni quita casos. El +1 sobre 344 es el guardarraíl del largo del statement (`content.test.ts`, `mediaText.body.length >= 155`). Antes, medido el 17 ago al cerrar el bloque de los salados: **344 en verde**. `npm run lint` y `npm run typecheck` limpios. Neto +2 sobre los 338 con que quedó el catálogo nuevo: los dos guardarraíles de `GALLERY_SLUGS` (que todos sus slugs existan, y que la lista rinda exactamente 24 fotos); +4 más por `revalidateRoute.test.ts`, el fichero 19. Los otros +5 sobre los 333 anteriores son del bloque de la tarjeta OG. |
| Tests e2e (19 ago, la tercera reseña real) | Medido **dos veces**, con la reseña resumida a 280 y otra vez con la literal de 400, las dos contra `NEXT_DIST_DIR=.next-verify` y build nuevo: `geometry.spec.ts` + `interactions.spec.ts` a los 8 anchos, **484 en verde + 52 skipped, cero rojos** de 536 en ambas (6,7 y 5,7 min). Los dos ficheros son los que vigilan este cambio —6 tarjetas, 30 estrellas y alturas idénticas entre las visibles (`geometry.spec.ts:321-354`), y el `--i` y «Mostrando reseñas 1 a N de 6» del slider (`interactions.spec.ts:448`, `:459`)— y no hacía falta la suite entera porque el diff es copy de una reseña: ni esquema, ni tipos, ni componentes, ni CSS. Los rojos de la línea base viven en `paginas.spec.ts` y `seo-perf.spec.ts`, que **no** entraron en esta selección: no son ausencia de fallo, son ausencia de medida. Unitarios **348 en verde** (sin casos nuevos), `typecheck` y `lint` limpios. Comprobado además contra el HTML prerenderizado: `Vanessa Manco` presente y **«Céspedes» ya no existe** en `.next-verify/server/app/`. Y con sonda propia de Playwright a cuatro anchos, que es lo que este cambio podía romper en silencio: **cero desbordes**, las **seis tarjetas idénticas** a 1440/992/767/390 y también en la última ventana del carrusel, la más larga (`t3`) dejando ~10px de crema. Con la cita de 400 la fila mide **533/579/336/525px** a 1440/992/767/390 —+81px sobre la de 280 en tres de los cuatro anchos, +54 a 767— y las de andamio quedan con 199-226px de crema a 992. El `role` de una línea **quita 27px** de esos altos: con «Piedades de Santa Ana» envolvía a dos y bajaba el párrafo de esa tarjeta respecto a las vecinas; con «Piedades» los tres párrafos arrancan a la misma altura, comprobado en captura a 992 y 390 |
| Tests e2e (17 ago, Web Analytics) | `paginas.spec.ts` a los 8 anchos contra `NEXT_DIST_DIR=.next-verify` y build nuevo: **112 en verde + 8 skipped + 8 en rojo** (1,7 min). **Los 8 son exactamente la línea base** —`paginas.spec.ts:290`, el aviso legal rinde 7 `.prose-block` y el test espera 6—, y el resto de ese mismo test **sí pasa con el copy nuevo**: la aserción de «No usamos cookies» (`:284`) va antes de la línea que falla. El componente se midió aparte, con una sonda de Playwright contra dos builds: con `VERCEL_ENV=production`, `document.head` recibe `<script defer src="/_vercel/insights/script.js" data-sdkn="@vercel/analytics/next" data-sdkv="2.0.1">` y sale **una** petición, sin ningún dominio de terceros; sin `VERCEL_ENV`, **cero scripts y cero peticiones**. ⚠ El componente devuelve `null` y añade el script desde un `useEffect`, así que **`curl` del HTML nunca lo ve**: esto se comprueba con navegador o no se comprueba |
...
| Lanzable | **sí, con una salvedad asumida.** La métrica ya es real, los precios y el dominio/hosting están cerrados y el sitio se sirve en `www.boquitacostarica.com`. Se lanza **con 3 de los 6 testimonios todavía de andamio**, por decisión explícita del 16 ago 2026: Ale los irá sustituyendo por tandas y el test los mantiene nombrados mientras tanto. La tanda del 19 ago cerró `t3`. Sigue abierta la panorámica original, que no bloquea. Ver [🔴](#-bloquea-el-lanzamiento) |

---

## Hecho

### El panel no mide solo, y comprobarlo tiene dos trampas

Encargo del 17 ago 2026. Web Analytics estaba activado en el panel de Vercel y las tarjetas de
visitas seguían a **0**: el interruptor del panel no mide nada por sí solo, hace falta que el código
monte el componente. Decidido con el cliente: **sólo Web Analytics** (Speed Insights queda fuera) y
**sólo en producción**.

- **La puerta**, en `lib/analytics.ts`: `isAnalyticsEnabled(vercelEnv = process.env.VERCEL_ENV)`.
  Tiene la forma de `isSiteIndexable()` (`lib/seo.ts:27`) —parámetro con valor por defecto para
  poder probarla sin tocar `process.env`— pero responde a otra pregunta, y por eso no se reutiliza:
  medir **no** depende de `SITE_LAUNCHED`. El sitio sigue cerrado a buscadores y aun así cuenta
  visitas. Sólo `production` porque el plan Hobby tiene tope mensual de eventos y las cifras del
  panel **no separan entornos**: un preview sumaría visitas de prueba a las reales.
- **En `app/layout.tsx`**, una línea al final del `<body>`: `{isAnalyticsEnabled() && <Analytics />}`.

**Lo que no era obvio (1): el componente no escribe nada en el HTML.** `<Analytics />` devuelve
`null` y añade el `<script>` desde un `useEffect` con `document.head.appendChild`
(`node_modules/@vercel/analytics/dist/next/index.mjs`). La primera verificación de este bloque fue
`curl | grep _vercel/insights` contra el build correcto y dio **vacío**, que se lee igual que «la
puerta está cerrada de más». Esto se comprueba **con navegador o no se comprueba**; la sonda de
Playwright, con las dos direcciones medidas, está en su fila de e2e.

**Lo que no era obvio (2): `npm i @vercel/analytics` no instala.** npm 11 intenta colocar
`@sveltejs/kit` —que el paquete declara como peer **opcional**— y eso arrastra `vite@8` contra el
`vite@7.3.6` de vitest: `ERESOLVE`. Nada de esto es Svelte real. Se instaló con `--legacy-peer-deps`
**una sola vez**, y el lockfile resultante es limpio: registra los peers opcionales del paquete y
**no** añade ni `@sveltejs/kit` ni `vite@8`. Lo que importa para el despliegue está medido: un
`npm install` **sin flags** posterior sale en verde y deja el lock intacto, así que Vercel instala
sin ayuda. ⚠ Quien reinstale desde cero borrando `package-lock.json` volverá a necesitar el flag.

**El aviso legal presumía de no medir.** `content/pages.ts` decía «No usamos cookies de seguimiento,
**ni analítica de terceros**, ni píxeles publicitarios», y en cuanto se activó la medición esa frase
quedó publicada y falsa. La sección pasa a «Cookies y medición de visitas» y cuenta lo que hay:
quién mide, que es agregado y sin cookies, qué campos registra, y que el identificador de visitante
único se renueva a diario sin guardar la IP. Se conservan el `id` `cookies` —es un ancla que puede
estar enlazada desde fuera— y las **siete** secciones, porque el texto nuevo es un párrafo dentro de
la que ya existía y no una sección más: así no se mueven ni `pages.test.ts:35` ni el conteo de
`.prose-block` de los e2e.

**Y el HTML viejo servido, otra vez.** El punto de `CLAUDE.md` sobre parar `next start` por PID
volvió a morder: el `kill` del job de bash no se llevó el proceso de Windows, el segundo
`next start` murió con `EADDRINUSE` **en su propio log** y la sonda midió el servidor anterior —el
del build con `VERCEL_ENV`, así que la puerta parecía abierta siempre—. Y el grep con el que se
comprobaba el puerto **estaba mal escrito**: en `netstat -ano` el puerto va **antes** de `LISTENING`,
de modo que `grep "LISTENING.*:3100"` no encuentra nunca nada y contestaba «libre» con el servidor
vivo. Lo fiable: `netstat -ano | grep ":3100"` y `taskkill //PID <pid> //F`.

**Verificado en producción — funciona, y el camino hasta ahí fue casi todo humo propio.** Medido
contra el dominio: `VERCEL_ENV` vale `production` y la puerta abre **en el build y en cada
petición**; el flight declara y usa el componente (`6:I[…,"Analytics"]` y `$L6` en el HTML de
`/tienda`); el `useEffect` corre —`window.va` es una función y `window.vam` vale `production`—; y el
script de analítica responde **200**.

**Dos trampas se comieron el diagnóstico, y las dos son del método de medición, no del sitio.** Están
escritas aquí porque cualquiera que vuelva a comprobar esto va a caer en las mismas:

1. **El script no se sirve desde `/_vercel/insights/script.js`.** En producción Vercel lo sirve desde
   una **ruta ofuscada de primera parte** —el día de la medición, `/75ca3850985ac2b3/script.js`— que
   configura él solo para esquivar los bloqueadores de anuncios. `/_vercel/insights/script.js` existe
   y responde 200, y es el camino que se usa en local, pero **no es el que pide el navegador en
   producción**. Las sondas filtraban el tráfico por la cadena `insights`, así que informaban de «cero
   peticiones» con el script cargando delante.
2. **El script se niega a medir en navegadores automatizados.** Su primera función es
   `t=()=>!!(navigator.webdriver||navigator.userAgent.includes("Headless"))`, y hace `if(t())return`
   antes de tocar nada. De ahí que `window.vaq` se quede con el `pageview` en cola y no salga ningún
   beacon: **Playwright no puede verificar esto, por diseño**, y las visitas de las sondas no cuentan
   en el panel. Se comprueba con un navegador de verdad.

⚠ **La guardia del lanzamiento está sana**, en contra de lo que este fichero afirmó durante un commit
(`bf23ecd`, corregido acto seguido). El andamio midió `VERCEL_ENV=production` y `SITE_LAUNCHED`
distinto de `"true"`: `isSiteIndexable()` devuelve `false` **por el motivo previsto** —el sitio no se
ha lanzado— y ponerlo en `true` lo abrirá. El punto que se había añadido a 🔴 se retiró.

**El andamio que rompió el bucle:** `app/api/diag-entorno` (`force-static`, se evalúa en el build) y
`diag-entorno-runtime` (`force-dynamic`, por petición), dos rutas gemelas para separar «qué vio el
build» de «qué ve el runtime». Las dos devolvieron `isAnalyticsEnabled: true`, y con eso la pregunta
dejó de ser «¿por qué no se monta?» para ser «¿por qué no veo el script?», que es la buena. Retiradas
en el commit siguiente; el patrón queda anotado por si vuelve a hacer falta.

⚠ **Sin cobertura: que el componente esté montado.** El unitario afirma la puerta, no el
`<Analytics />` del layout — `vitest` corre en `environment: "node"` e incluye sólo `.ts`, así que no
puede renderizarlo. Un e2e sí podría, pero necesitaría arrancar el build con `VERCEL_ENV=production`
y el `webServer` de `playwright.config.ts` no pasa ese entorno. Hoy lo sostiene la sonda manual de
este bloque.
...
Encargo de copy del 17 ago 2026, sobre la sección `catalogo` de `/sobre-nosotros`
(`content/pages.ts:129`). Sólo texto: ni estructura, ni CSS, ni catálogo de la tienda.

| Dónde | Qué cambia |
| --- | --- |
| «Queques:» | Zanahoria y limón salen de la lista de punto y coma y pasan a frase propia: «no es el típico», el frosting de queso crema en cobertura **y capitas de relleno**, y el de limón «esponjoso y cítrico». La enumeración la abre ahora Devil's Food |
| «Galletas:» | Los polvorones estrenan «cuando los llevas a la boca se deshacen» y por eso dejan de encabezar la enumeración: se quedan con su frase y las otras dos galletas siguen encadenadas |
| «Postres:» | El key lime pie explica por qué **nunca queda blanco** —la yema rojiza de los huevos de la feria tiñe el pie de naranja— y se escribe en minúsculas, como se llama el producto en `content/products.ts:603`. El cheese cake aclara que **el coulis va aparte**, que es información de servicio |

**El punto del pie de brigadeiro era un no-op.** Ale pidió quitarle «esto es el edén del chocolate»,
y esa frase **no existía en el repo** (`grep -rn "edén"` → 0 resultados): el párrafo ya decía
exactamente lo que ella pedía. Venía de un borrador anterior suyo, así que no se tocó nada por ahí.

Los retoques de puntuación y concordancia van consultados con el cliente, no por cuenta propia: el
texto entraba en medio de listas ya escritas y mezclaba puntos con punto y coma. También se corrigió
un «nunca queda pero» que era un desliz de dictado en la frase del key lime pie.

**Sin tests nuevos, y no por olvido.** Lo que este bloque podía romper ya está cubierto: las cinco
entradillas y su orden (`tests/unit/pages.test.ts:100`, con `toEqual`), la clave de React por los 30
primeros caracteres (`:91`) y la frontera entre entradilla y texto (`:125`). Los tres párrafos nuevos
pasan las tres. `npm test` **345 en verde**, `typecheck` limpio.

Sincronizado el documento fuente `docs/boquita-sobre-nosotros.md`, por lo que dice la viñeta del
apellido en «Páginas de texto y límites de error»: lo que se queda ahí vuelve al sitio en el próximo
repaso de contenido. ⚠ **El resto de ese documento sigue divergido de antes** —dice «diecisiete
recetas» donde el sitio dice veinte, y «las cuatro entradillas» donde hay cinco—: no se arregló en
este bloque.
...
- **Tres de las seis reseñas ya son reales**: `t1` es María Elena M. (Escazú) y `t2` es
  Mirella S. (Santa Ana), las dos del 15 ago, y `t3` es **Vanessa Manco** (Piedades de Santa Ana),
  del 19 ago, que sustituyó a la inventada «Laura Céspedes Mora». `t4`…`t6` siguen siendo andamio
  con `todo: true`, así que `CONTENT_TODO §3` sigue bloqueando: **faltan tres**. El original sin
  editar de cada una, lo que se cambió al publicarlo y el estado del permiso están en
  `docs/TESTIMONIOS_FUENTES.md`. El consentimiento de las tres **está concedido**: Ale confirmó las
  dos primeras el 16 ago 2026 y la de Vanessa el 19 ago 2026.
- ⚠ **`t3` se publica LITERAL y por eso subió el tope del esquema: `quote` pasa de 320 a 400.** Son
  los dos últimos párrafos de lo que escribió Vanessa, tal cual, con su ❤️ —la única tarjeta con
  emoji del sitio— y **gastando los 400 exactos**, así que añadirle un carácter rompe el test a
  propósito. Hubo una versión resumida a 280 publicada unas horas el 19 ago (`92be347`); el cliente
  prefirió sus palabras. El tope viejo se justificaba contra unos `min-height` de `.review-card` que
  **ya no existen** desde D-41, así que subirlo no rompe nada: el límite es de composición, y lo que
  cuesta cada tramo está medido en el comentario de `content/schema.ts`.
- ⚠ **Su cita afirma «sin aditivos químicos»**, que es una afirmación sobre el producto y no sólo una
  opinión de sabor. Va en boca de ella y con su permiso, pero es **el único sitio del sitio donde
  aparece**: si algún día hay que sostenerlo, se sabe dónde está.
- **El `role` dice «Piedades», no «Piedades de Santa Ana», por composición.** Con el distrito
  completo son 42 caracteres —pasa de los 40 del esquema— y salía en **dos líneas** a 992, bajando su
  párrafo 27px respecto a las vecinas. Medido: hasta ~29 caracteres cabe en una línea a 992 y 390.
- **«queque» era palabra de ella**, pero en los párrafos que **no** se publican. El fragmento
  publicado no nombra ningún producto, así que no hay nada que ajustar al vocabulario del sitio.
...
- **El guardarraíl cambió de lado dos veces.** Primero un test exigía `items.length === 0`; luego,
  ...
  tests se funden en uno que deja de contar y **nombra**: hoy `["t4","t5","t6"]`, tras entrar `t3` el
  19 ago. Sustituir un texto sin quitar su marca —o quitar una marca sin sustituir el texto— sigue
  rompiéndolo, y cuando llegue la última la lista queda vacía.
...
- **El alto de la tarjeta ya no lo mandan los `min-height` del spec.** Eran cinco números
  ...
  levanta ahora las seis tarjetas juntas. **Re-medido el 19 ago** con `t3` literal (400 car.), que es
  ya la más larga con diferencia: **cero desbordes** y las **seis tarjetas idénticas** a 1440, 992,
  767 y 390, también en la última ventana del carrusel; la más larga deja **~10px** de crema, que es
  el residuo de la caja de línea y no depende de qué reseña manda. El coste, medido sobre el build a
  tres largos de `t3`: a 992 la fila mide **579px** con 400 caracteres, 471 con 280 y 417 con 238; a
  390, **525 / 417 / 363**. O sea **~27px de fila por cada ~60 caracteres**, y la pagan las seis
  tarjetas. La contrapartida está en las cortas: a 992 las de andamio quedan con **199-226px** de
  crema bajo su texto y a 390 con 172-199px. Es la propiedad de siempre —igualar altura no depende de
  `--per-view`—, amplificada por una cita larga; si algún día molesta, **lo que se toca es el largo
  del copy, no el CSS**, y el sitio donde está escrito el precio de cada tramo es el comentario de
  `content/schema.ts`.
...
- **El aviso legal describe la medición real, así que copy y código quedan atados.** La sección
  `cookies` de `content/pages.ts` cuenta lo que hace Vercel Web Analytics. Si algún día se retira
  el `<Analytics />` de `app/layout.tsx`, o se añade otra herramienta, ese texto miente en la
  dirección contraria y no falla nada: `tests/unit/pages.test.ts` sólo vigila que no se vuelva al
...
| `docs/implementation_tasks.md:235`, `:404`, `:887-888` (UI-058, conflicto C-3) | «la sección está oculta esperando testimonios reales» | doblemente obsoleto: se publica desde el 13 ago, y desde el 19 ago **tres** de las seis reseñas son reales |
