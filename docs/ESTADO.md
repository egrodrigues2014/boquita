...
| **Bloque Open Graph/SEO, sin commitear** | Está a medias en el árbol de trabajo (`app/opengraph-image.tsx`, `app/og-hero.jpg`, `scripts/build-images.mjs`, `lib/seo.ts`, la `description` de `app/layout.tsx`) y **deja 8 rojos en la suite**: al quitar `streetAddress` del JSON-LD nadie actualizó `seo-perf.spec.ts:24`, que todavía lo exige. Cerrar el bloque incluye decidir qué afirma ese test sobre la dirección |
