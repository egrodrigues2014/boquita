...
- ⚠ **`t3` es la primera reseña que se publica RESUMIDA, y es la más larga de las seis.** El
  original de Vanessa medía ~980 caracteres contra el tope de 320 de `content/schema.ts`, así que lo
  publicado son **280** que conservan sus cinco argumentos —cuatro años de clienta, el queque de
  zanahoria, la textura casera, los polvorones españoles y los ingredientes naturales con el azúcar
  cuidada—, sin atribuirle ningún producto que no nombrara. Se atenuó «sin aditivos químicos» a
  «con ingredientes naturales y cuidando el azúcar»: en la portada una frase así funciona como
  reclamo del producto, y el sitio no lo afirma en ningún otro sitio. Su `role` gasta los **40
  caracteres exactos** del tope, o sea que retocarlo obliga a acortarlo. Detalle completo de la
  compresión en su ficha de `docs/TESTIMONIOS_FUENTES.md`.
- **«queque» en `t3` es palabra de ella**, no una corrección nuestra: a diferencia de `t1` y `t2`,
  esta reseña ya usaba el vocabulario del sitio.
...
- **El alto de la tarjeta ya no lo mandan los `min-height` del spec.** Eran cinco números
  ...
  levanta ahora las seis tarjetas juntas. **Re-medido el 19 ago**, al entrar `t3` con 280 caracteres
  y pasar a ser ella la más larga: **cero desbordes** y las **seis tarjetas idénticas** a 1440, 992,
  767 y 390, también en la última ventana del carrusel; la más larga sigue dejando **~10px** de
  crema, que es el residuo de la caja de línea y no depende de qué reseña manda. Y el coste, que es
  el dato nuevo: la fila **sube 81px** a 1440/1280/992/390 (tres líneas más), 27px a 500 y 22px a
  767, donde antes mandaba el suelo de 260px. La contrapartida está en las cortas: a 390, con una
  tarjeta por vista, la más corta (`t2`, 190 car.) pasa a mostrar **118px** de crema bajo su texto.
  Es la propiedad de siempre —igualar altura no depende de `--per-view`—, amplificada por una reseña
  más larga; si algún día molesta, lo que se toca es el largo del copy, no el CSS.
