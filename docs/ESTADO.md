...
⚠ **Falta la confirmación en producción**, que no se puede hacer desde el repo: tras el despliegue,
que `/_vercel/insights/script.js` responda **200** —en local da 404 y el paquete lo avisa por
consola, porque ese endpoint sólo existe dentro de Vercel— y que el panel deje de estar a 0, que
tarda ~30 s. Si sigue a 0 con la petición saliendo, es un bloqueador de contenido del navegador:
filtran `/_vercel/insights`.
