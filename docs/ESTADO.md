...
| Sin commitear | **El bloque de las dos reseñas reales + estrellas**, sin commitear al escribir esto: `content/home.ts`, `components/ui/StarRating.tsx` (nuevo), `components/sections/Testimonials.tsx`, `components/sections/TestimonialsSlider.tsx`, `styles/17-testimonials.css`, `docs/TESTIMONIOS_FUENTES.md` (nuevo), `tests/unit/content.test.ts`, `tests/unit/contrast.test.ts`, `tests/e2e/geometry.spec.ts`, `tests/e2e/interactions.spec.ts` y documentación. Verificar con `git status --short`. ⚠ `scripts/verify-staging-order.ts` aparece sin rastrear y **no es de este bloque**: no meterlo en el mismo commit. ⚠ Tampoco es de ese bloque el borrado del apellido («Ale Budowski» → «Ale»): comparte `content/home.ts` y `tests/`, pero además toca `content/pages.ts`, `lib/seo.ts`, `app/sobre-nosotros/page.tsx` y `docs/boquita-sobre-nosotros.md`. |
...
| Tests unitarios | **333 en verde**, 18 ficheros (`npm test`, medido el 15 ago al cerrar el bloque de reseñas). `npm run lint` y `npm run typecheck` limpios. Neto +1 sobre los 332 anteriores: dos comprobaciones nuevas de contraste para las estrellas (`--gold-line` sobre crema ≥3:1, y `--gold` sobre crema por debajo de 3:1, que es el motivo de que la estrella lleve contorno) y los dos tests de andamio fundidos en uno. |
...
| Lanzable | **no**: faltan **4 de los 6 testimonios** (los otros 2 ya son reales, con el permiso pendiente de confirmar), métrica defendible, panorámica original y dominio/hosting. Los precios ya **no** bloquean. Ver [🔴](#-bloquea-el-lanzamiento) |
...
- **Dos de las seis reseñas ya son reales** (15 ago): `t1` es María Elena M. (Escazú) y `t2` es
  ...
  `docs/TESTIMONIOS_FUENTES.md`. ⚠ El consentimiento de las dos publicadas **está pendiente de
  confirmar con Ale**, y van con nombre en una web pública.
...
| | Qué falta | Dónde |
| --- | --- | --- |
| 1 | **Faltan 4 de los 6 testimonios**, con nombre/ocasión/texto. **Hay dos reales desde el 15 ago** —María Elena M. y Mirella S., publicadas sin marca— y su original, edición y permiso están en `docs/TESTIMONIOS_FUENTES.md`. ⚠ **El consentimiento de esas dos está pendiente de confirmar** y ya van con nombre en público: es lo primero que hay que cerrar. Para las cuatro que faltan sigue faltando el texto fuente: Instagram no es legible desde el PDF ni desde este entorno sin login. Lo que impide colar el andamio como real es un test que ya no cuenta marcas sino que **nombra** las que quedan (`["t3","t4","t5","t6"]`) | `CONTENT_TODO.md §3` |
...
| **Despliegue** | La preparación de código está cerrada para Vercel y `boquitacostarica.com`. Falta crear/importar el proyecto, cargar secretos, validar la URL temporal con `SITE_LAUNCHED=false` y, sólo al pasar a Pro, cambiar los registros A/CNAME en Arsys y abrir la indexación. Hobby se usará únicamente para validación interna no comercial. |
