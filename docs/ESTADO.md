...
| Desplegado | **sí, y cerrado a buscadores.** `https://www.boquitacostarica.com` responde 200 sirviendo el build actual, y el ápex redirige con 308 a `www`. Sigue en `noindex, nofollow, nocache` y con `Disallow: /` en `robots.txt`, que es exactamente lo que impone la guardia `SITE_LAUNCHED` de `lib/seo.ts`. Medido el 16 ago contra el dominio; la redacción anterior («no, no hay proyecto de Vercel») estaba obsoleta. **Desde el 17 ago mide visitas con Vercel Web Analytics**, y sólo en producción (`lib/analytics.ts`): estar en `noindex` no impide contar quién llega. ⚠ **Medido el 17 ago contra el dominio y NO mide todavía:** `/_vercel/insights/script.js` responde 200 pero el script no se pide nunca, porque en ese despliegue `VERCEL_ENV` no vale `production`. Ver el punto 3 de [🔴](#-bloquea-el-lanzamiento), que es la parte grave |
...
### Web Analytics medía cero, porque el panel no mide solo
...
⚠ **Medido en producción el mismo día, y ahí está la sorpresa.** Con `d636e41` desplegado,
`/_vercel/insights/script.js` responde **200** —Web Analytics está bien activado en el proyecto— y el
copy nuevo del aviso legal se sirve, así que el despliegue es el correcto. Pero **el script no se pide
nunca**: la sonda de Playwright contra el dominio da **cero scripts y cero peticiones**, igual en la
portada (estática, decide en el build) que en `/tienda` (dinámica, decide en cada petición). O sea que
en ese despliegue **`VERCEL_ENV` no vale `production`**, y la puerta cierra con razón.

No es el código ni es la analítica: son las **variables de sistema**, que no llegan al proyecto. Lo
que descarta la explicación fácil es que **las variables propias sí llegan** —el canonical sirve
`boquitacostarica.com` y no `localhost`, así que `NEXT_PUBLIC_SITE_URL` está puesta—. Señal
secundaria: los assets **no llevan `?dpl=`**, la marca del deployment ID que Vercel deja de exponer
cuando esa casilla está apagada. La casilla es **«Enable access to System Environment Variables»**
(Settings → Environment Variables); en la documentación vieja se llamaba «Automatically expose System
Environment Variables», y por eso no se encuentra buscando ese nombre. Al activarla hace falta
**redesplegar**: la portada es estática y hornea el valor en el build.

⚠ **Y eso arrastra la guardia del lanzamiento, que es peor que la métrica.** Ver el punto 3 de
[🔴 Bloquea el lanzamiento](#-bloquea-el-lanzamiento).
...
El 1 y el 2 no son código —detalle completo en `docs/CONTENT_TODO.md`—. El **3 apareció el 17 ago**, es configuración de Vercel y no tiene fichero que arreglar.
...
| 3 | **La guardia de indexación no puede abrirse hoy, y falla en silencio.** `isSiteIndexable()` (`lib/seo.ts:31`) exige `VERCEL_ENV === "production"` **y** `SITE_LAUNCHED === "true"`, y el 17 ago se midió que en el despliegue de producción **`VERCEL_ENV` no vale `production`** (ver «Web Analytics medía cero»). Mientras siga así, poner `SITE_LAUNCHED=true` el día del lanzamiento **deja el sitio en `noindex`** y nada avisa: los dos síntomas —la analítica que no mide y la indexación que no abre— son el mismo problema. Se arregla en el panel, no en el repo: casilla «Enable access to System Environment Variables» + redespliegue | `lib/seo.ts:27`, panel de Vercel |
