...
| Tests unitarios | **344 en verde**, 19 ficheros (`npm test`, medido el 17 ago al cerrar el bloque de los salados). `npm run lint` y `npm run typecheck` limpios. Neto +2 sobre los 338 con que quedó el catálogo nuevo: los dos guardarraíles de `GALLERY_SLUGS` (que todos sus slugs existan, y que la lista rinda exactamente 24 fotos); +4 más por `revalidateRoute.test.ts`, el fichero 19. Los otros +5 sobre los 333 anteriores son del bloque de la tarjeta OG. |
